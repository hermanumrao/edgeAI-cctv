...
// CSV writer thread – safe, no weird try_lock()
...
void rotate_segment_nonblocking(int width, int height) {
    ...
    if (old_filename.empty()) old_filename = g_current_filename;

    // Move events to pending CSV queue (copy, not move-from)
    std::vector<Event> copy_events;
    {
        std::lock_guard<std::mutex> ek(g_evt_mtx);
        if (!g_segment_events.empty()) {
            copy_events = g_segment_events; // copy
            g_segment_events.clear();
        }
    }
    if (!copy_events.empty()) {
        std::lock_guard<std::mutex> pk(g_pending_csv_mtx);
        g_pending_csv.push_back({ old_filename, copy_events });
    }
}
...
void capture_thread() {
	...
	cv::VideoCapture cap(
    "libcamerasrc ! video/x-raw,format=NV12,width=640,height=480,framerate=20/1 ! appsink",
    cv::CAP_GSTREAMER
);
...
cv::cvtColor(frame, frame, cv::COLOR_YUV2BGR_NV12);
        ...
        if (hog_frame_counter >= HOG_INTERVAL) {
            ...
            g_hog.detectMultiScale(gray, found, 0.3, cv::Size(16,16), cv::Size(32,32), 1.10, 2);
            if (!found.empty()) {
                humanish_hog = true;
                for (auto &p : found) {
        ...
        // Combine detectors => event uses both
    ...
    if (RECORD_ENABLED) {
        ...
        std::vector<Event> copy_events;
        {
            std::lock_guard<std::mutex> ek(g_evt_mtx);
            if (!g_segment_events.empty()) {
                copy_events = g_segment_events;
                g_segment_events.clear();
            }
        }
        if (!copy_events.empty()) {
            std::lock_guard<std::mutex> pk(g_pending_csv_mtx);
            g_pending_csv.push_back({ g_current_filename, copy_events });
        }
    }
    cap.release();
}
...
int main(int argc, char** argv) {
    ...
    g_running = false;
    if (httpthr.joinable()) httpthr.detach(); // accept may block; process is exiting anyway
